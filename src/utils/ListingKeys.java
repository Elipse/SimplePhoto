...
//        System.out.println("" + ((JTabbedPane) component).getUI());
        ...
        list(inputMap, inputMap.keys());
        list(inputMap, inputMap.allKeys());

        System.out.println("JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT");
        inputMap = component.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
        list(inputMap, inputMap.keys());
        list(inputMap, inputMap.allKeys());

        System.out.println("JComponent.WHEN_IN_FOCUSED_WINDOW");
...
//        javax.swing.plaf.basic.BasicTabbedPaneUI;
    ...
    public static Map<String, Action> listAct(JComponent c) {
        if (c == null) {
        ...
        Map<String, Action> efecMap = new HashMap<String, Action>();

        for (int i = 0; i < keys.length; i++) {
            // Get the action name bound to this keystroke
            while (actionMap.get(keys[i]) == null) {
                actionMap = actionMap.getParent();
            }

            Action value = actionMap.get(keys[i]);
            if (value != null) {
                System.out.println("m:" + keys[i] + ":" + value);
                efecMap.put(keys[i].toString(), value);
            }
        }
        return efecMap;
    }
    ...
    public static void list(InputMap map, KeyStroke[] keys) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (map.get(keys[i]) instanceof String) {
                actionName = (String) map.get(keys[i]);
            } else {
                action = (Action) map.get(keys[i]);
            }

            System.out.println("keystrokeStr " + keystrokeStr + "-" + actionName + "-" + action);
        }
    }
